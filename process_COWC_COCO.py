...
def make_img_cutouts(image_path,image_mask_path,save_directory,cutout_size,size_output,windowsize,rotate,mask_margin_test,maketestdata=True,sample_annotation = None):
    ...
    global coco_anno_train
    global coco_anno_val
    global format_img_record
    global format_anno_record
        ...
        for H in range(H_slot+1):
            if H != H_slot or use_edge:
                for W in range(W_slot+1):
                    ...
                    if write_flag:
                        write_flag = False
                        bbox_ = decode_mask2bbox(cutout_mask_,windowsize,zero_based=True)
                        if len(bbox_) > 0:  #omit if there is no car
                            ...
                            for i in range(img_number-n_angles+1,img_number+1):
                                filename = filename_prefix + "_" + usage + year + "_" + str(i).zfill(12) + '.jpg'
                                ...
                                #output process
                                anno_dest = coco_anno_train if usage == "train" else coco_anno_val
                                format_img_record["license"] = 3
                                format_img_record["id"] = i
                                format_img_record["file_name"] = filename
                                format_img_record["width"] = size_output
                                format_img_record["height"] = size_output
                                anno_dest["images"].append(format_img_record.copy())

                                for n_anno, bbox_s in enumerate(bbox):
                                    bbox_s[2:] = [windowsize*size_output/cutout_size]*2
                                    format_anno_record["id"] = annotation_id + n_anno
                                    format_anno_record["category_id"] = 3 #car
                                    format_anno_record["image_id"] = i
                                    format_anno_record["iscrowd"] = 0
                                    format_anno_record["bbox"] = bbox_s.copy()
                                    anno_dest["annotations"].append(format_anno_record.copy())
                                annotation_id += len(bbox)

                                cv.imwrite(os.path.join(save_directory,usage+year,filename),cutout)
...
if __name__ == "__main__":
    ...
    for dir_ in dir_lists:
        ...
        elif record[0] == "sample_annotation" : sample_annotation = record[1]
        else:
            try:
    ...
    if not os.path.isdir(os.path.join(save_directory,"train" + year)):
        os.makedirs(os.path.join(save_directory,"train" + year))
    if maketestdata:
        if not os.path.isdir(os.path.join(save_directory, "val" + year)):
            os.makedirs(os.path.join(save_directory, "val" + year))
    if not os.path.isdir(os.path.join(save_directory, "annotations")):
        os.makedirs(os.path.join(save_directory, "annotations"))
    ...
    coco_anno_file = open(sample_annotation)
    coco_anno_train = json.load(coco_anno_file)
    coco_anno_file.close()

    format_img_record = coco_anno_train["images"][0].copy()
    format_anno_record = coco_anno_train["annotations"][0].copy()

    coco_anno_train["images"] = []
    coco_anno_train["annotations"] = []

    coco_anno_val = copy.deepcopy(coco_anno_train)
    ...
    for directory in process_directories:
        ...
        for image_path, image_mask_path in zip(image_list,image_mask_list):
            ...
            make_img_cutouts(image_path, image_mask_path, save_directory, cutout_size, output_size, windowsize, rotate, mask_margin_test, maketestdata, sample_annotation)

    coco_anno_file_train = open(os.path.join(save_directory, "annotations", "instances_train" + year + ".json"),
                                "w")
    coco_anno_file_val = open(os.path.join(save_directory, "annotations", "instances_val" + year + ".json"), "w")
    json.dump(coco_anno_train, coco_anno_file_train)
    json.dump(coco_anno_val, coco_anno_file_val)
    coco_anno_file_train.close()
    coco_anno_file_val.close()
