...
def _elementwise_softmax_cross_entropy(x, t,raw):
    if not raw:
        ...
        x_p = F.softmax(x,axis=2)
        t_p = F.softmax(t,axis=2)
        return F.sum(F.log(x_p)*t_p,axis=2)*(-1.)
...
def multibox_loss(mb_locs, mb_confs, gt_mb_locs, gt_mb_labels, k,raw=False):
    ...
    if raw:
        labels = F.argmax(gt_mb_labels,axis=2)
    else:
        labels = gt_mb_labels
    positive = labels.array > 0
    ...
    conf_loss = _elementwise_softmax_cross_entropy(mb_confs, gt_mb_labels,raw)
